...
import type { BuildPromptInputFile } from "./types.js";
...
function parseArgs(argv: string[]): Args {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (a === "--completion-stdout") args.completionToStdout = true;
    else if (a === "--quiet-prompt") args.quietPrompt = true;
    else pos.push(a);
  }
...
function readJsonFile(p: string): BuildPromptInputFile {
  const raw = fs.readFileSync(p, "utf8");
  const parsed = JSON.parse(raw);
  if (!parsed || typeof parsed !== "object") throw new Error("JSON must be an object");
  if (!Array.isArray((parsed as any).examples) || typeof (parsed as any).input !== "string") {
    throw new Error('Expected shape: { "examples": [...], "input": string }');
  }
  return parsed as BuildPromptInputFile;
}
...
export async function main(argv = process.argv.slice(2)): Promise<void> {
  ...
  let payload: BuildPromptInputFile;
  try {
